...
当前桌面源码版本 **0.4.3**，定位为有明确支持范围的工程验证版。固定圆柱、翼型、喷管已生成十万级网格；任意复杂几何的鲁棒性、网格无关性与 CFD 精度仍需逐例验证。

新增自研 **二维稳态不可压层流**求解器：最终多边形网格 → SIMPLE / Rhie–Chow → 速度、运动学压力与守恒报告。桌面可设置工况、启动/取消、查看流场并导出；另保留扩散/泊松验证 CLI。当前限定矩形外域绕流、矩形通道和顶盖腔，默认一阶迎风，另有试验性的限制线性重构，**桌面尚不支持非定常设置，求解器不支持湍流或任意入口/出口布置**。新增求解功能本轮仅在 macOS 实测；下方既有三平台下载包不含这次新功能。 [构建、运行与能力范围](docs/DEVELOPMENT_CN.md#自研求解器入口)

开发分支的 **CLI/核心已加入非定常不可压层流**：真实物理时间、一阶后向欧拉、CFL诊断和断点续算；每个时间步都检查动量与质量守恒，未收敛时保留最后接受的状态。解析涡衰减、规则/扭曲网格及启动流动用于验证。该功能尚未接入桌面0.4.3，也不代表湍流能力。[运行方式](docs/DEVELOPMENT_CN.md#非定常层流与断点续算) · [真实网格及误差图](artifacts/current/native-flow-transient.png)

开发分支已加入固定稀疏结构和压力 IC(0) 预条件，完成 **112,368 / 500,472 格层流通道**的真实求解与独立解析/守恒检查。它们是指定工况的规模证据，尚不代表复杂绕流或湍流能力；已随0.4.1重新打包到本地 macOS 应用；该规模计时来自此前性能阶段，不代表新格式的同规模资格。[耗时、内存与能力边界](docs/CURRENT_STATE_CN.md#当前性能改进固定稀疏结构与-ic0)

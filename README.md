...
开发分支新增**被动标量／恒物性温度输运核心与CLI**：从已保存的流场读取共享面通量，支持稳态和后向欧拉、定值和绝热/指定通量边界，独立重算守恒与细化误差。当前CLI冻结载流，尚未接入温度桌面操作、同步热流耦合或浮力；不属于下方安装包的新功能。[真实温度场](artifacts/current/native-scalar-transport.png) · [运行与边界单位](docs/DEVELOPMENT_CN.md#守恒标量与恒物性热输运) · [验证范围](docs/CURRENT_STATE_CN.md#当前改进守恒标量与恒物性热输运核心)

...
SST-2003m核心现有真实壁距、梯度/应变重建、解析壁面k/omega条件和**固定载流上的非线性输运**。两个域各三档实际网格通过返回场的独立模型、面通量及原方程收支审核；另在102,400格上验证全部壁距，模块单次约.065秒，这不是CFD求解耗时。**速度/压力耦合、标准湍流工况和近壁精度仍未验收，App尚未开放湍流。** [实际场与验证边界](docs/CURRENT_STATE_CN.md#当前改进非线性sst输运与解析壁面边界)

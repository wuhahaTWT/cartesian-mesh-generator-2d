...
main 保留参与构建和测试的实验 Euler/SST 代码，其资格与上述层流功能分开。本 worktree 的 `codex/compressible-flow` 按用户授权独立推进 Euler：新增可选 HLLC/HLLE、受限二阶及实际桌面入口，数值证据和未完成项以 [当前状态](docs/CURRENT_STATE_CN.md) 为准；未合入 main。

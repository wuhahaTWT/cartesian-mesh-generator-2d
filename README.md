...
开发分支进一步复用压力多重网格分组，每次仍重算当前矩阵系数。指定十万格涡流单步总耗时约185.1→178.3秒，解析误差几乎不变，独立守恒审核通过；圆柱没有显示提速。桌面包未更新，原热输运规模验证仍待完成。[实测范围与限制](docs/CURRENT_STATE_CN.md#当前改进压力多重网格分组复用)

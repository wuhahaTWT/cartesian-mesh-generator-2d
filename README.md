...
当前桌面源码版本 **0.4.26**，定位为有明确支持范围的工程验证版。固定圆柱、翼型、喷管已生成十万级网格；任意复杂几何的鲁棒性、网格无关性与 CFD 精度仍需逐例验证。
...
0.4.26 在App显示每个命名边界的流入、流出、净流量和平均法向速度；从实际守恒面通量积分，单位为二维每单位厚度的m²/s。对同一喷管新增统计前后，实际场和残差历史逐字节一致。[真实流量表](artifacts/current/native-boundary-flux-app.png)

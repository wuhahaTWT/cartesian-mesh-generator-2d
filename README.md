...
原生二维笛卡尔网格生成器，提供 **macOS、Windows、Linux 桌面应用**和命令行工具。导入二维轮廓或 PNG/JPG 图片，设置网格数量或相对尺寸，生成纯笛卡尔背景网格、真实 Cut-cell 或贴体边界层网格。背景模式保留模型内部完整单元；后两种流体网格可导出 OpenFOAM case。

当前源码版本见 [desktop/package.json](desktop/package.json)，实际打包与验收范围以[当前状态](docs/CURRENT_STATE_CN.md)为准。项目定位为有明确支持范围的工程验证版。固定圆柱、翼型、喷管已生成十万级网格；任意复杂几何的鲁棒性、网格无关性与 CFD 精度仍需逐例验证。

自研 **二维稳态／非定常不可压层流**求解器直接使用最终多边形网格：SIMPLE / Rhie–Chow、共享面通量、完整黏性应力、后向欧拉，以及可选的面限制线性重构。App已接入内流/外流、逐面命名速度入口、压力出口与轴对齐静压开口、对称边界、静止/移动壁面、旋转同心圆环、固定/自动时间步、初始局部涡、取消续算、逐边界流量、受力/CFL监测、保存/读取流动工况和真实场导出。工况文件绑定已有网格，尚不包含完整几何/网格项目；湍流仍未在App开放。另有实验性理想气体 Euler 核心/CLI及桌面入口；本轮只收尾不可压层流，不拓展可压、湍流或新的流动形式。本轮原生CFD测试已在三平台通过，桌面CFD端到端交互在macOS实测；三平台打包烟雾覆盖网格与背景模式。下方旧三平台下载包不包含本轮更新，最新0.4.38构建附件见[已通过的三平台CI](https://github.com/wuhahaTWT/cartesian-mesh-generator-2d/actions/runs/35667584541)。[构建、运行与能力范围](docs/DEVELOPMENT_CN.md#自研求解器入口)

实验性 **理想气体 Euler** 已直接使用最终二维多边形网格，推进密度、两分量动量和总能量，带声学 CFL、正值检查、周期/滑移/特征远场及检查点。精确 Sod 冲击管和独立通量守恒复核通过；**等熵涡精度研究未通过，一阶格式耗散较大**，不作为工程可压精度资格。已有实验桌面入口，仍受上述精度限制，本轮不扩展。[实际场与失败记录](artifacts/current/native-euler-foundation.png) · [运行方式](docs/DEVELOPMENT_CN.md#实验性理想气体-euler)

当前App已实际跑通喷管、旋转圆环、圆柱、厚翼型和双柱；每份结果分别检查网格和离散方程，不把“收敛”当成精度合格。0.4.23的自动模式可在粗候选明确质量失败后有界细化重试，厚翼型从失败候选得到4,728格有效网格并完成计算，数量偏差仍明确显示。[真实厚翼型App](artifacts/current/native-airfoil-app.png) · [验证范围与剩余问题](docs/CURRENT_STATE_CN.md)
...
0.4.38缓存固定重构几何，保留原运算顺序与全部停止门。同一102,017格曲壁喷管，重新测量Cholesky完整求解471.70→392.48秒，场/残差逐字节相同；计入同一粗解和映射，较原aggregation流程840.87→411.37秒。峰值内存有增加，任意喷管物理精度仍未认证。[完整性能与实际网格](artifacts/current/native-laminar-gradient-cache.png) · [验证范围](docs/CURRENT_STATE_CN.md#当前推荐版本0438固定几何缓存与完整交付)

以下版本记录保留各自当时的验证范围，不替代当前验收。

0.4.28 加入可选的**稳态迭代加速**：在流动设置中选择“历史迭代加速（Anderson）”。同一Mac、相同输入和停止门，半通道43.84→7.57秒、喷管10.31→1.72秒、圆柱2.10→.78秒；候选场需降低原动量残差并守恒，最终仍由普通SIMPLE迭代确认。默认关闭，不用于非定常/温度联算；另外修复更严容差下线性求解先停止更新的问题。[真实网格、场差与计算成本](artifacts/current/native-steady-acceleration.png)

0.4.27 修复稳态面通量对速度松弛参数的依赖；默认仍为0.6，非定常计算路径保持原公式。0.4.26 在App显示每个命名边界的流入、流出、净流量和平均法向速度；从实际守恒面通量积分，单位为二维每单位厚度的m²/s。对同一喷管新增统计前后，实际场和残差历史逐字节一致。[真实流量表](artifacts/current/native-boundary-flux-app.png)

0.4.25 加入**命名对称边界和半通道模板**，复用已有矩形模型。三档规则网格呈二阶速度趋势，1,300格真实App网格通过守恒和导出审核，流量误差.2091%；需要16,523轮才能达到1e-9，原10,000轮未收敛保留。仅支持水平／竖直对称面。[实际半通道App](artifacts/current/native-symmetry-app.png) · [独立参考与限制](artifacts/current/native-symmetry-flow.json)
...
当前目标是补齐纯笛卡尔背景网格、提高高密度不可压层流计算速度，并收尾已有桌面计算和导出流程。保持原几何、质量和精度要求，不拓展新的流动形式；[当前目标与真实验证范围](docs/CURRENT_STATE_CN.md)持续更新。
...
3. **计算（可选）**：生成后，在“原生层流”选择匹配工况（纯笛卡尔背景模式不进入求解），填写运动黏度、速度和迭代上限，启动求解；完成后可切换速度/压力色图。
...
- **纯笛卡尔背景**：保留内部、外部和相交的完整单元，导出分类JSON/VTK/PNG；无CM2D流体拓扑或OpenFOAM case。
- **原生二维Cut-cell**：四叉树控制疏密，物面处保留真实流体 Cut-cell 多边形；OpenFOAM 输出将二维网格挤出为单层体单元，前后边界为 `empty`。
- **贴体边界层（Beta）**：壁面四边形层与笛卡尔余域共形连接，支持外流和单环内流；带孔内流目前使用纯 Cut-cell。
- **流体网格结果 ZIP**：包含 CM2D、VTK、JSON 质量/参数报告、输入轮廓、OpenFOAM case，以及白底全景＋局部放大的 `mesh-preview.png` 和中文文件说明。图片导入还保留原图、确认轮廓图与标定记录。

...
当前桌面源码版本 **0.4.8**，定位为有明确支持范围的工程验证版。固定圆柱、翼型、喷管已生成十万级网格；任意复杂几何的鲁棒性、网格无关性与 CFD 精度仍需逐例验证。
...
0.4.8 修复了部分壁面切割单元压力重构放大误差的问题：默认约5千格圆柱已完成流动与温度推进、取消续算和导出。它解决的是已复现的数值失稳，尚未增加物理出口回流模型。[真实网格与修复说明](artifacts/current/pressure-stencil.png) · [本轮验证](docs/CURRENT_STATE_CN.md#当前改进壁面压力重构与默认圆柱-048)

...
当前桌面源码版本 **0.4.28**，定位为有明确支持范围的工程验证版。固定圆柱、翼型、喷管已生成十万级网格；任意复杂几何的鲁棒性、网格无关性与 CFD 精度仍需逐例验证。

自研 **二维稳态／非定常不可压层流**求解器直接使用最终多边形网格：SIMPLE / Rhie–Chow、共享面通量、完整黏性应力、后向欧拉，以及可选的面限制线性重构。App已接入内流/外流、逐面命名速度入口、压力出口与轴对齐静压开口、对称边界、静止/移动壁面、旋转同心圆环、固定/自动时间步、初始局部涡、取消续算、逐边界流量、受力/CFL监测、保存/读取流动工况和真实场导出。工况文件绑定已有网格，尚不包含完整几何/网格项目；湍流仍未在App开放。另有实验性理想气体 Euler 核心/CLI，可压 App、高阶重构及黏性/热传导尚未实现。新增求解功能仅在macOS实测，下方旧三平台下载包不包含本轮更新。[构建、运行与能力范围](docs/DEVELOPMENT_CN.md#自研求解器入口)

实验性 **理想气体 Euler** 已直接使用最终二维多边形网格，推进密度、两分量动量和总能量，带声学 CFL、正值检查、周期/滑移/特征远场及检查点。精确 Sod 冲击管和独立通量守恒复核通过；**等熵涡精度研究未通过，一阶格式耗散较大**，不作为工程可压精度资格。此入口尚未接入当前 App。[实际场与失败记录](artifacts/current/native-euler-foundation.png) · [运行方式](docs/DEVELOPMENT_CN.md#实验性理想气体-euler)

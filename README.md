...
当前桌面版本 **0.4.0**，定位为有明确支持范围的工程验证版。固定圆柱、翼型、喷管已生成十万级网格；任意复杂几何的鲁棒性、网格无关性与 CFD 精度仍需逐例验证。

新增自研 **二维稳态不可压层流**求解器：最终多边形网格 → SIMPLE / Rhie–Chow → 速度、运动学压力与守恒报告。桌面可设置工况、启动/取消、查看流场并导出；另保留扩散/泊松验证 CLI。当前限定矩形外域绕流、矩形通道和顶盖腔，使用一阶迎风对流，**不支持湍流、非定常或任意入口/出口布置**。新增求解功能本轮仅在 macOS 实测；下方既有三平台下载包不含这次新功能。 [构建、运行与能力范围](docs/DEVELOPMENT_CN.md#自研求解器入口)

开发分支已加入固定稀疏结构和压力 IC(0) 预条件，完成 **112,368 / 500,472 格层流通道**的真实求解与独立解析/守恒检查。它们是指定工况的规模证据，尚不代表复杂绕流或湍流能力；此次性能改动尚未重新打入桌面安装包。[耗时、内存与能力边界](docs/CURRENT_STATE_CN.md#当前性能改进固定稀疏结构与-ic0)

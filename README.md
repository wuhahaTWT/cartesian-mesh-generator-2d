...
近壁法向加密已推进到1,024格诊断平板：y+约.15–.45，原先失败的案例通过独立最终场审核。求解器显式保留候选低位，并在写回double后重新验收；2,048格仍在omega输运检查失败，未放宽容差。这是低Re核心诊断，尚未验收高Re湍流或十万格SST性能。[真实网格、精度与失败边界](docs/CURRENT_STATE_CN.md#当前改进显式高低位候选与最终场精度复核)

...
SST诊断入口现支持实际速度、黏度、入口湍流量、前缘及矩形域。已跑板长Re=5千、5万和1千万；4份流场通过独立方程审核，另1份薄网格因梯度复算不一致被拒绝并保留。**高Re近壁分辨率及摩阻仍未验收**，没有把“求解完成”当成工程准确度证明。[物理输入、真实结果与失败](docs/CURRENT_STATE_CN.md#当前改进可配置物理输入与高雷诺数诊断)

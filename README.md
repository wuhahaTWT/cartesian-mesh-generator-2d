...
当前桌面源码版本 **0.4.10**，定位为有明确支持范围的工程验证版。固定圆柱、翼型、喷管已生成十万级网格；任意复杂几何的鲁棒性、网格无关性与 CFD 精度仍需逐例验证。
...
开发CLI新增可选瞬态速度松弛与同步热涡规模验证：同一四万格两步，显式.8约57秒，默认.6约107秒，独立守恒审核通过；**十万格仍未在预算内完成两步**，.9/1.0试验失败，默认值未改。已区分时间与空间参考误差，不能用该短时案例宣称工程热流或十万格资格完成。[实测、失败及适用范围](docs/CURRENT_STATE_CN.md#当前改进同步热输运规模对照与数值控制)

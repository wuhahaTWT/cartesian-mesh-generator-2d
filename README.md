...
当前桌面源码版本 **0.4.11**，定位为有明确支持范围的工程验证版。固定圆柱、翼型、喷管已生成十万级网格；任意复杂几何的鲁棒性、网格无关性与 CFD 精度仍需逐例验证。

新增自研 **二维稳态／非定常不可压层流**求解器：最终多边形网格 → SIMPLE / Rhie–Chow → 速度、运动学压力与守恒报告。桌面可设置工况、启动/取消、查看流场并导出；另保留扩散/泊松验证 CLI。当前限定矩形外域绕流、矩形通道和顶盖腔，默认一阶迎风，另有试验性的限制线性重构，**求解器不支持湍流或任意入口/出口布置**。新增求解功能本轮仅在 macOS 实测；下方既有三平台下载包不含这次新功能。 [构建、运行与能力范围](docs/DEVELOPMENT_CN.md#自研求解器入口)
...
已补充真实壁段距离、k/omega梯度和二维应变重建：六个空间变化的冻结SST案例通过独立面通量/守恒审核。另在102,400格上验证全部壁距，模块单次约.065秒；这不是CFD求解耗时。完整物理近壁处理与RANS耦合仍待实现。[实际网格与证据](docs/CURRENT_STATE_CN.md#当前改进真实壁面距离与sst空间重建)

SST-2003m已实现局部模型与k/omega冻结输运基础，隐式耗散及两方程均匀衰减通过真实网格和时间步细化验证。**完整RANS耦合、近壁处理和工程湍流验证仍未完成，App尚未开放湍流。** [实测曲线与边界](docs/CURRENT_STATE_CN.md#当前改进sst-2003m输运基础与隐式耗散)

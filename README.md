...
当前桌面源码版本 **0.4.12**，定位为有明确支持范围的工程验证版。固定圆柱、翼型、喷管已生成十万级网格；任意复杂几何的鲁棒性、网格无关性与 CFD 精度仍需逐例验证。

新增自研 **二维稳态／非定常不可压层流**求解器：最终多边形网格 → SIMPLE / Rhie–Chow → 速度、运动学压力与守恒报告。桌面可设置工况、启动/取消、查看流场并导出；另保留扩散/泊松验证 CLI。当前桌面支持矩形外域绕流、矩形通道、顶盖腔，以及左右竖直开口的曲壁通道／喷管，默认一阶迎风，另有试验性的限制线性重构。**桌面尚不支持湍流或任意入口/出口布置**。新增求解功能本轮仅在 macOS 实测；下方既有三平台下载包不含这次新功能。 [构建、运行与能力范围](docs/DEVELOPMENT_CN.md#自研求解器入口)

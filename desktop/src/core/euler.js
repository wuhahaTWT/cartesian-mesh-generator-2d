...
function validateEulerRequest(input) {
  ...
  requireValue(r.endTime>0&&r.minimumStep>0&&r.maximumStep>=r.minimumStep&&r.cfl>0&&r.cfl<=.45,'时间步与声学/导热/黏性组合 CFL 无效。');
...
function eulerCheckpoint(text,mesh,input) {
  ...
  if(version===3){const mu=lines.filter(line=>line.startsWith('VISCOSITY '));requireValue(mu.length===1&&Number(mu[0].split(' ')[1])===r.dynamicViscosity,'检查点动力黏度不匹配。');}
...
function validateEulerOutput(summary,fields,cellsText,facesText,historyText,checkpointText,mesh,input,startTime=0) {
  ...
  for(let i=0;i<raw.length;i++) {
    ...
    const rate=viscous?numbers(row,['viscousRate'])[0]:0;requireValue(rate>=0&&(r.dynamicViscosity>0||rate===0),'黏性步长系数错误。');viscousRates.push(rate);
  }
  ...
  faceRows.forEach((row,i)=>{
    ...
    const stress=viscous?numbers(row,['viscousMomentumX','viscousMomentumY','viscousWork']):[0,0,0];
    if(viscous){const convective=numbers(row,['convectiveMomentumX','convectiveMomentumY']);requireValue(convective.every((v,k)=>near(v+stress[k],flux[k+1])),'黏性/对流动量通量分解错误。');}
    requireValue(r.dynamicViscosity>0||stress.every(v=>v===0),'零黏度存在黏性通量。');
    if(e.neighbour<0&&Number(row.partner)<0)boundaryViscousWork+=stress[2];
    if(row.kind==='no-slip-wall')requireValue(r.wallModel==='no-slip'&&stress[2]===0,'静止无滑移壁功或类型错误。');
    if(thermal) {
      const [heat,convective]=numbers(row,['heatFlux','convectiveEnergy']);requireValue(near(heat+convective+stress[2],flux[3])&&(r.thermalConductivity>0||heat===0),'总能量/导热通量分解错误。');
      if(e.neighbour<0&&Number(row.partner)<0)boundaryHeat+=heat;
      if(['slip-wall','no-slip-wall'].includes(row.kind))requireValue(flux[0]===0&&near(flux[3],heat+stress[2]),'热壁质量或能量通量错误。');
    }
  ...
  let maximumCellBalanceRelative=0,acousticCourant=0,thermalCourant=0,viscousCourant=0,combinedCourant=0;
  for(let i=0;i<current.length;i++) {
    acousticCourant=Math.max(acousticCourant,s.lastStep*spectral[i]/areas[i]);
    thermalCourant=Math.max(thermalCourant,s.lastStep*heatRates[i]);viscousCourant=Math.max(viscousCourant,s.lastStep*viscousRates[i]);combinedCourant=Math.max(combinedCourant,s.lastStep*(spectral[i]/areas[i]+heatRates[i]+viscousRates[i]));
    for(let k=0;k<4;k++) {
  ...
  if(viscous)requireValue(near(history.at(-1).viscousCourant,viscousCourant)&&near(history.at(-1).boundaryViscousWork,boundaryViscousWork),'历史黏性终态错误。');
  ...
  return {summary:s,fields,history,request:r,audit:{maximumCellBalanceRelative,historyMaximumEnergyBalanceRelative,acousticCourant,thermalCourant,viscousCourant,combinedCourant,boundaryHeat,boundaryViscousWork,scope:'last accepted step finite-volume balance, EOS, mesh/field/checkpoint binding; full selected numerical flux and RK stages audited separately'}};
}

...
void exportResult(const std::filesystem::path& dir,const im::Grid& g,const im::Result& r,const std::string& caseName) {
    ...
    us<<"i,j,x,y,u,solid_mask\n";vs<<"i,j,x,y,v,solid_mask\n";
    ...
    for(std::size_t j=0;j<c.ny;++j)for(std::size_t i=0;i<c.nx;++i) {
        ...
        us<<i<<','<<j<<','<<static_cast<double>(i)*g.dx()<<','<<y<<','<<s.u[k]<<','<<g.maskU[k]<<'\n';
    }
    for(std::size_t j=0;j<=c.ny;++j)for(std::size_t i=0;i<c.nx;++i) {
        const auto k=g.index(i,j);
        vs<<i<<','<<j<<','<<(static_cast<double>(i)+.5)*g.dx()<<','<<static_cast<double>(j)*g.dy()<<','<<s.v[k]<<','<<g.maskV[k]<<'\n';
    }
    ...
    report<<"{\n\"format\":\"cartmesh2d-immersed-flow-v1\",\n\"solver\":\"periodic-mac-brinkman\",\n\"case\":"<<jsonString(caseName)<<",\n\"experimental\":true,\n\"product_solver_ready\":false,\n"
          <<"\"boundary_conditions\":{\"x\":\"periodic velocity and pressure fluctuation\",\"y\":\"stationary no-slip\",\"solid\":\"finite Brinkman penalty\"},\n"
...
int main(int argc,char** argv) {
    try {
        ...
        for(int i=1;i<argc;++i) {
            ...
            if(key=="--help") { std::cout<<"Experimental native 2D uniform Cartesian MAC/Brinkman steady flow\n"
                ...
                <<"--steady-tolerance R --continuity-tolerance R --linear-tolerance R --linear-solver ic0|cholesky\n"
            ...
            if(key=="--output")outputDir=value;else if(key=="--case")caseName=value;else if(key=="--boundary")boundary=value;
                ...
                if(value!="ic0"&&value!="cholesky")throw std::runtime_error("unknown linear solver");c.systemCholesky=value=="cholesky";
        ...
        if(c.systemCholesky && setenv("VECLIB_MAXIMUM_THREADS","1",1)!=0)
        ...
        history<<"step,pseudo_time,dt,momentum,continuity,field_change,pressure_linear_residual,pressure_linear_rhs_norm,pressure_iterations\n";
        std::signal(SIGINT,cancel);std::signal(SIGTERM,cancel);
        const auto result=im::solve(grid,[&](const im::State& s,const im::Metrics& m) {
            history<<s.steps<<','<<s.pseudoTime<<','<<m.dt<<','<<m.momentum<<','<<m.continuity<<','<<m.fieldChange<<','<<m.pressureLinearResidual<<','<<m.pressureLinearRhsNorm<<','<<m.pressureIterations<<'\n';

...
| 需求 | 实读位置与深度 | 取什么 | 等级 |
|---|---|---|---|
| 一致的退化边塌缩 | `OpenFOAM/.../edgeCollapser.H:285-297,308-336`（实读接口：collapse 在 coupled boundary 间一致，并分 small edge / merge edge / small-sliver face）；`edgeCollapser.C:800-920`（实读 priority 选保留点与 collapse string 打断）；`applications/.../collapseEdges.C:28-46`（实读功能说明与坏面回退） | **W1 只取架构原则，独立实现**：拓扑改动必须在共享 identity 空间一致提交，优先保留不可移动 wall sample，失败后 fail closed。本项目的 `f*h` 判据是自身 Q1 无量纲合同推导，不是复制 OpenFOAM 的阈值或代码；二维实现也没有采用其 polyTopoChange | D |

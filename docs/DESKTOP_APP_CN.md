...
三种画法：`层级配色`（按层级/尺寸档上色，看尺寸场一眼就懂）、`白底线框` 和
`暗底线框`（只画线，适合出图）。线框模式下 ramp 图例自动收掉，只留边界色键。

**手动加密区**是"非必要加密"的出口。物面附近那一圈由尺寸场保证，不需要也不应该手动
调；尾流、下游关注区、想看清楚的地方则是判断题，所以列在面板里逐个编辑：坐标以体长为
单位、相对物体中心，深度写成"低于壁面 N 级"。画布上用虚线画出来，并按 `refine()` 的
行为裁剪到计算域——画在域外的框不会产生加密，overlay 就不该承诺它。

这需要 `cartmesh2d_cli` 放开一条限制：原来 `--size-field` 和 `--refine-box` 互斥。
现在 `--refine-box` 是**叠加**的（尺寸场自己就为尾迹生成 box region，用户放的区域是同一
个原语），`--distance-band` 仍然互斥，因为它会和尺寸场刚建好的壁面阶梯打架。

## 7. 层级着色的依据不同

纯路径的 `sourceKey` 是 Quadtree key，低六位就是层级。**hybrid 路径不是**：
`HybridMesh2D.cpp` 写的是 `sourceKey = sourceId`，一个流水号。按它取模 64 上色会在
一个对称几何上画出 id-mod-64 的噪声——圆柱看起来花花绿绿就是这个原因。所以 hybrid
按单元面积分档（`sqrt(area)` 每翻一倍一档），圆柱因此重新变成同心的。

直方图和图例的标题跟着依据变：`每层级单元数` 对 `每尺寸档单元数`。

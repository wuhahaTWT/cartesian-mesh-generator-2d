...
`core/` 八个文件分别管能力、样例、几何/SVG 输入、参数、尺寸预算、CM2D、结果摘要。
`renderer/app.js` 管表单和状态，`viewport.js` 管真实网格画布；HTML/CSS 管布局。没有第二套前端。
...
可加 `--method=hybrid`、`--mode=light`、`--regions=1`。smoke 会走真实表单/IPC/CLI 后退出；它不等于所有界面操作都已验收。

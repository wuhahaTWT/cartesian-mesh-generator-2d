...
|---|---|---|---|
| 纯 Cut-cell（`cartmesh2d_cli`） | **level 9** | 13024 solver cells、20188 leaves、0.54 s | level 10：边界折线擦过格点切出 `3.5e-8` 微面，boundary skewness 8.6 > 4 |
...
纯 Cut-cell 到 level 9 时 non-orthogonality 随加密**变好**（68.6° → 42.3°），
说明这条路径的瓶颈是单点几何退化，不是整体质量退化。

基线 `artifacts/r2/w0-baseline-manifest.json`，测量入口
...
| Q2-A | 关闭 | 共享交点构造进入 hybrid 默认路径；纯 Cut-cell 路径**尚未**接入 |
...
| R2/W1 | **进行中，有阻塞** | 焦合机制已落地；`cartmesh2d_cli` 迁到共享构造仍未完成 |
| R2/W2、W3、W4 | 未开始 | 见 `docs/R2_HANDOFF_CN.md` |
...
所以这是**随 level 必然恶化**的失败模式。机制已就绪
（`gridCornerWeldFractionOfLocalH`，默认不变），迁移未完成。

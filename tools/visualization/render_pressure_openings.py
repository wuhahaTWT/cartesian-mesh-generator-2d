...
def render(root,output):
    ...
    half='halfChannelVelocity' in report['reference'];drop=report['reference']['pressureDrop']
    cases=report['cases'];fine=cases[-1];prefix=root/f"n{fine['n']}";path=Path(str(prefix)+'.solver.cm2d')
    audit=native.verify_case(path,prefix,'custom',.1,1,native.argument_parser().parse_args(['--max-iterations','12000' if half else '10000']))
    ...
    ax.add_collection(collection);ax.autoscale_view();ax.set_aspect('equal');ax.set_title(f"Actual {fine['cells']:,}-cell velocity field and mesh | static p/rho: {drop:g} -> 0")
    fig.colorbar(collection,ax=ax,label='u (m/s)',shrink=.75);ax.set_xlabel('x (m)');ax.set_ylabel('y (m)')
    ax=fig.add_subplot(gs[1,0]);ys=np.linspace(0,1,201);ax.plot(1.5*ys*(2-ys) if half else 6*ys*(1-ys),ys,'k--',label='analytic')
    for case in cases:
    ...
    ax=fig.add_subplot(gs[1,1]);ax.plot([r['x'] for r in rows],[r['p'] for r in rows],'.',ms=1,label='actual cell pressure');ax.plot([0,4],[drop,0],'k--',label='analytic');ax.legend(fontsize=8);ax.set_xlabel('x (m)');ax.set_ylabel('p/rho (m2/s2)');ax.set_title('Static pressure, not total pressure')
    ax=fig.add_subplot(gs[2,0]);h=[1/c['n'] for c in cases];u=[c['velocityL2'] for c in cases];q=[c['flowError'] if half else c['flowRelativeError'] for c in cases]
    ...
    verification_note=(f"Free-slip acceleration max error: {report['plug']['maxVelocityError']:.3g} m/s" if half else 'Restart: byte-identical; equal pressure: rest')
    ax=fig.add_subplot(gs[2,1]);ax.axis('off');ax.text(0,.96,f"Fine velocity L2: {fine['velocityL2']:.6g} m/s\nFine flow rate: {fine['flow'] if half else fine['flowPerDepth']:.9f} m2/s (exact 1)\nFlow rate error: {(fine['flowError'] if half else fine['flowRelativeError'])*100:.4f}%\nMax pressure error: {fine['pressureMaxError']:.3g} m2/s2\nVelocity observed order: {fine['observedOrder'] if half else fine['observedVelocityOrder']:.4f}\nIndependent momentum / continuity: PASS\n{verification_note}\n\nReference scope: this rectangular laminar channel.\nDoes not qualify curved ports or general CFD accuracy.",va='top',linespacing=1.55)
    fig.suptitle('Pressure-driven half-channel | top symmetry, bottom no-slip | 0.4.25' if half else 'Pressure-driven incompressible flow | native solver 0.4.24',fontsize=15);output.parent.mkdir(parents=True,exist_ok=True);fig.savefig(output,dpi=170);plt.close(fig)

...
def main():
    ...
    p.add_argument('--timeout',type=float,default=180.,help='Per-process resource budget, at most 600 s; independent of numerical acceptance')
    ...
    a=p.parse_args()
    if not math.isfinite(a.timeout) or not 0<a.timeout<=600:p.error('timeout must be in (0,600] seconds')

#!/usr/bin/env python3
"""Independent read-back of the Cartesian MAC/Brinkman analysis files.
...
def verify(directory, require_converged=False):
    ...
    for j in range(ny):
        for i in range(nx):
            ...
            residual = c['viscosity']*lap-advx-advy-(P(i,j)-P(i-1,j))/hx-us[k]['solid_mask']*U(i,j)/c['penalty_time']+c['drive']
    ...
    for j in range(1,ny):
        for i in range(nx):
            ...
            residual = c['viscosity']*lap-advx-advy-(P(i,j)-P(i,j-1))/hy-vs[j*nx+i]['solid_mask']*V(i,j)/c['penalty_time']
            momentum = max(momentum, abs(residual)/scale)
    ...
    for index,loop in enumerate(loops):
        ...
        segments.extend(zip(loop,loop[1:]+loop[:1]))
    ...
    for wall in walls:
        require(min(distance((wall['x'],wall['y']),a,b) for a,b in segments)<=1e-11*max(g['length'],g['height']), 'wall sample is not on the original polygon')
